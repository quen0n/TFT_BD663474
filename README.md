...
* [FAQ](https://github.com/RainbowKiwiFOX/TFT_BD663474#FAQ)
## Общее описание
...
- **```TFT_setTextBackColor(uint16_t color)```** - установить цвет фона текста. TFT_COLOR_none - прозрачный
#### Служебные функции
...
2) Недостаточное или зашумлённое питание. Попробуйте продублировать провода питания или припаять конденсатор на 1000 мкФ по питанию прямо на гребёнку дисплея.
3) Неправильно указаны макросы для работы с портами или неправильно настроен SPI.

##### Q: Дисплей работает настабильно, появляются артефакты и т.д.
...
A: В ```BD663474.h``` поменяйте значения макросов для работы с портами ввода/вывода и переделайте функцию TFT_sendCmd() под ваше железо. Можно переключиться на программный SPI если хотите быстрее портироваться.
